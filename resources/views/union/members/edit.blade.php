        ...
        <form action="{{ route('union.members.update', $worker->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <flux:input name="name" :label="__('Nombre completo')" type="text" required autocomplete="name"
                    maxlength="120" value="{{ old('name', $worker->name) }}" placeholder="Juan Pérez" />

                <flux:input name="email" :label="__('Correo electrónico')" type="email" required
                    autocomplete="email" maxlength="120" value="{{ old('email', $worker->email) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <flux:input name="curp" :label="__('CURP')" type="text" required maxlength="18"
                    value="{{ old('curp', $worker->curp) }}" placeholder="PEGA850101HDFRRN09" x-data
                    x-on:input="$el.value = $el.value.toUpperCase()" />

                <flux:input name="rfc" :label="__('RFC')" type="text" required maxlength="13"
                    value="{{ old('rfc', $worker->rfc) }}" placeholder="PEGA850101XXX" x-data
                    x-on:input="$el.value = $el.value.toUpperCase()" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <flux:select name="gender" :label="__('Sexo / Género')" required>
                    ...
                    <option value="ND" {{ old('gender', $worker->gender) == 'ND' ? 'selected' : '' }}>No definido
                    </option>
                    <option value="X" {{ old('gender', $worker->gender) == 'X' ? 'selected' : '' }}>Prefiero no
                        decirlo</option>
                </flux:select>

                <flux:input name="budget_key" :label="__('Clave presupuestal')" type="text" required maxlength="30"
                    value="{{ old('budget_key', $worker->budget_key) }}" placeholder="123-ABC" />

            </div>

            <flux:select name="active" :label="__('Estado')" required>
                <option value="">Selecciona</option>
                <option value="1" {{ old('active', (int) $worker->active) == 1 ? 'selected' : '' }}>Activo
                </option>
                <option value="0" {{ old('active', (int) $worker->active) == 0 ? 'selected' : '' }}>Inactivo
                </option>
            </flux:select>
